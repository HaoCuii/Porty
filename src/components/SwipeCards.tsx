...
const SwipeCards = ({ className }: SwipeCardsProps) => {
  ...
  return (
    <div
      className={cn(
        "relative grid h-[320px] w-[240px] place-items-center",
        className,
      )}
    >
      {cards.length === 0 && (
...
const Card = ({
  ...
  const isFirstCard = id === 1; // First card (portfolio5.jpg)
  ...
  return (
    <motion.img
      src={url}
      alt="Placeholder alt"
      className="absolute h-[320px] w-[240px] origin-bottom rounded-lg bg-white object-cover hover:cursor-grab active:cursor-grabbing"
      style={{
        gridRow: 1,
        gridColumn: 1,
        x,
        opacity,
        rotate,
        objectPosition: isFirstCard ? "center 30%" : "center center",
        transform: isFirstCard ? "scale(1.2)" : undefined,
        boxShadow: isFront
